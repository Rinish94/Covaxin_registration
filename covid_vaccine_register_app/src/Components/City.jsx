...
function City() {
 ...
 useEffect(() => {
  setLoading(true);
  fetch(
   `https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/calendarByDistrict?district_id=670&date=${fDate}`
  )
   .then((res) => res.json())
   .then((res) => {
    setData(res.centers);
  ...
  <div className="container">
   ...
   <div className="filter">
    ...
    <select value={selectedCountry} onChange={changeCountry}>
     ...
     {result.map((x, i) => {
      return (
       <option value={x.pincode} style={{ textAlign: "left" }} key={i}>
        {x.block} {x.pincode}
       </option>
      );
     })}
        ...
        if (selectedCountry == "all") {
         return (
          <tr style={{ marginBottom: "10px" }} key={unitIndex + 1}>
           <td style={{ textAlign: "left", borderBottom: "1px solid brown" }}>
            {item.name}
           </td>
           <td style={{ borderBottom: "1px solid brown" }}>
            {unit.min_age_limit}
           </td>
           <td
            style={{
             color: unit.available_capacity === 0 ? "red" : "green",
             marginLeft: "38px",
             textAlign: "right",
